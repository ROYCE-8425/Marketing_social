...
const customFetch = (url, options = {}) => {
  let targetUrl = url;
  const isSupabaseCloud = typeof targetUrl === 'string' && targetUrl.includes('supabase.co');
  if (!isSupabaseCloud && typeof targetUrl === 'string') {
    targetUrl = targetUrl.replace('/rest/v1/', '/');
  }
  const opts = { ...options };
  if (!isSupabaseCloud && (SUPA_KEY === 'dev-no-auth' || !SUPA_KEY)) {
    if (opts.headers) {
